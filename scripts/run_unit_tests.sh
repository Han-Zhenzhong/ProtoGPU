...
if command -v ctest >/dev/null 2>&1; then
  ...
  ctest --test-dir "$BUILD_DIR" -C "$CONFIG" -V -R "^(gpu-sim-(tests|inst-desc-tests|simt-tests|memory-tests|observability-contract-tests|public-api-tests|builtins-tests|config-parse-tests|tiny-gpt2-mincov-tests)|cudart-shim-(smoke-tests|streaming-tests))$"
  exit 0
fi
