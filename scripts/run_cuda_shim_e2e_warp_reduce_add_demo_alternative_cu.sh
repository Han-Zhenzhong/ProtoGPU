...
if [[ $TOOLCHAIN_OK -eq 1 ]]; then
  ...
  gpusim_cuda_compile_host_binary "cudart_warp_reduce_add_cu" "$EXE_SRC" "$BIN" "$OUT_DIR" || exit $?

  echo "[cudart-shim-warp-reduce-add-cu] generating text PTX for override"
  gpusim_cuda_generate_ptx "cudart_warp_reduce_add_cu" "$PTX_SRC" "$PTX" "$OUT_DIR" || exit $?
else
...
if [[ $RC -ne 0 ]]; then
  echo "error: warp_reduce_add_demo failed (exit=$RC)" >&2
...
if ! grep -Eq "^OK$|\bOK\b" "$STDOUT"; then
  echo "error: warp_reduce_add_demo did not print OK" >&2
  echo "--- stdout ($STDOUT) ---" >&2
  tail -n 200 "$STDOUT" >&2 || true
  echo "--- stderr ($STDERR) ---" >&2
  tail -n 200 "$STDERR" >&2 || true
