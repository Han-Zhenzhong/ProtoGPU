  ...
  const std::string config = R"JSON(
{
  ...
  std::vector<protogpu_arg_desc> args = {
    ...
    protogpu_arg_desc{ PROTOGPU_ARG_U32, 0, 77, protogpu_blob_ref{ 0, 0 } },
  };
  ...
  auto result = protogpu::execute_blocking_job_remote(socket_path, job, arena, buffers);
  if (result.status != PROTOGPU_STATUS_OK) {
    std::cerr << "remote submit failed: " << result.diagnostic << "\n";
    return 1;
  }

  std::cout << "remote-submit result: " << unpack_u32_le(buffers[0].bytes) << "\n";
  return 0;
}
